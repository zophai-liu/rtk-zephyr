...
#define RTL87X2G_DMA_CTLR(id, dir)           DT_INST_DMAS_CTLR_BY_NAME(id, dir)
#define RTL87X2G_DMA_CHANNEL_CONFIG(id, dir) DT_INST_DMAS_CELL_BY_NAME(id, dir, config)
...
#define RTL87X2G_DMA_CONFIG_SOURCE_DATA_SIZE(config) (1 << ((config >> 6) & 0x3))

/* destination data size defined on bits 8-9 */
/* 0 -> 1 byte, 1 -> 2 bytes, 2 -> 4 bytes */
#define RTL87X2G_DMA_CONFIG_DESTINATION_DATA_SIZE(config) (1 << ((config >> 8) & 0x3))
