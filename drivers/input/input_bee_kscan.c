...
struct kscan_bee_data {
	...
	uint32_t key_map[CONFIG_BEE_INPUT_KEYSCAN_MAX_ROW_SIZE];
	uint8_t press_num;
	bool cb_en;
	uint16_t press_rows;
#ifdef CONFIG_PM_DEVICE
...
#if CONFIG_BEE_INPUT_KEYSCAN_GHOST_KEY_FILTER
...
static void kscan_bee_isr(void)
{
	...
	uint32_t new_key_map[CONFIG_BEE_INPUT_KEYSCAN_MAX_ROW_SIZE];
