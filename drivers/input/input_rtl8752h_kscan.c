...
static int kscan_rtl8752h_init_driver(const struct device *dev, uint32_t scanmode,
				      uint32_t manual_sel)
{
	LOG_DBG("dev %s init\n", dev->name);
...
static void manual_kscan_timer_cb(struct k_timer *timer)
{
	...
	kscan_rtl8752h_init_driver(dev, KeyScan_Manual_Scan_Mode, KeyScan_Manual_Sel_Bit);
}
#endif
	...
	if (KeyScan_GetFlagState(keyscan, KEYSCAN_INT_FLAG_SCAN_END) == SET) {
		...
		if (new_press_num == 0) {
			...
			k_timer_start(&manual_kscan_timer,
				      K_MSEC(CONFIG_RTL8752H_KEYSCAN_MANUAL_SCAN_INTERVAL_MSEC),
				      K_FOREVER);
		}
#endif
		...
		if (wakeup_flag) {
			if (data->press_rows & BIT(j)) {
				if (state->pins[i].wakeup_high) {
					Pad_ControlSelectValue(state->pins[i].pin, PAD_SW_MODE);
					System_WakeUpPinEnable(state->pins[i].pin,
							       PAD_WAKEUP_POL_LOW, DISABLE, 0);
				} else if (state->pins[i].wakeup_low) {
					Pad_ControlSelectValue(state->pins[i].pin, PAD_SW_MODE);
					System_WakeUpPinEnable(state->pins[i].pin,
							       PAD_WAKEUP_POL_HIGH, DISABLE, 0);
				}
			} else {
				if (state->pins[i].wakeup_high) {
					Pad_ControlSelectValue(state->pins[i].pin, PAD_SW_MODE);
...
static int kscan_rtl8752h_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_RESUME:
		(void)clock_control_on(RTL8752H_CLOCK_CONTROLLER,
...
#else
			kscan_rtl8752h_init_driver(dev, KeyScan_Auto_Scan_Mode,
						   KeyScan_Manual_Sel_Key);
#endif
		} else {
			KEYSCAN_DLPSExit(keyscan, &data->store_buf);
