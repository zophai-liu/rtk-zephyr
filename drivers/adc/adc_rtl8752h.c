...
#define DT_DRV_COMPAT realtek_rtl8752h_adc
...
LOG_MODULE_REGISTER(adc_rtl8752h, CONFIG_ADC_LOG_LEVEL);

struct adc_rtl8752h_config {
	uint32_t reg;
	uint16_t clkid;
	uint8_t channels;
...
static int adc_rtl8752h_start_read(const struct device *dev, const struct adc_sequence *sequence)
{
	struct adc_rtl8752h_data *data = dev->data;
	const struct adc_rtl8752h_config *cfg = dev->config;
...
static int adc_rtl8752h_read(const struct device *dev, const struct adc_sequence *seq)
{
	struct adc_rtl8752h_data *data = dev->data;
	int error;

	adc_context_lock(&data->ctx, false, NULL);
	error = adc_rtl8752h_start_read(dev, seq);
...
static int adc_rtl8752h_read_async(const struct device *dev, const struct adc_sequence *sequence,
				   struct k_poll_signal *async)
{
	...
	error = adc_rtl8752h_start_read(dev, sequence);
	adc_context_release(&data->ctx, error);
...
static int adc_rtl8752h_channel_setup(const struct device *dev,
...
{
	struct adc_rtl8752h_data *data = CONTAINER_OF(ctx, struct adc_rtl8752h_data, ctx);
...
static void adc_rtl8752h_isr(const struct device *dev)
{
	struct adc_rtl8752h_data *data = dev->data;
	const struct adc_rtl8752h_config *cfg = dev->config;
	ADC_TypeDef *adc = (ADC_TypeDef *)cfg->reg;
...
#ifdef CONFIG_PM_DEVICE
static int adc_rtl8752h_pm_action(const struct device *dev, enum pm_device_action action)
{
	struct adc_rtl8752h_data *data = dev->data;
	const struct adc_rtl8752h_config *cfg = dev->config;
	ADC_TypeDef *adc = (ADC_TypeDef *)cfg->reg;
...
static const struct adc_driver_api adc_rtl8752h_driver_api = {
	.channel_setup = adc_rtl8752h_channel_setup,
	.read = adc_rtl8752h_read,
#ifdef CONFIG_ADC_ASYNC
	.read_async = adc_rtl8752h_read_async,
#endif /* CONFIG_ADC_ASYNC */
};

static int adc_rtl8752h_init(const struct device *dev)
{
	struct adc_rtl8752h_data *data = dev->data;
	const struct adc_rtl8752h_config *cfg = dev->config;
	int ret;
...
#define RTL8752H_ADC_INIT(index)                                                                   \
	PINCTRL_DT_INST_DEFINE(index);                                                             \
	static void adc_rtl8752h_irq_config_func(const struct device *dev)                         \
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN(index), DT_INST_IRQ(index, priority), adc_rtl8752h_isr,   \
			    DEVICE_DT_INST_GET(index), 0);                                         \
		irq_enable(DT_INST_IRQN(index));                                                   \
	}                                                                                          \
	static struct adc_rtl8752h_data adc_rtl8752h_data_##index = {                              \
		ADC_CONTEXT_INIT_TIMER(adc_rtl8752h_data_##index, ctx),                            \
		ADC_CONTEXT_INIT_LOCK(adc_rtl8752h_data_##index, ctx),                             \
		ADC_CONTEXT_INIT_SYNC(adc_rtl8752h_data_##index, ctx),                             \
		.is_bypass_mode = DT_INST_PROP(index, is_bypass_mode),                             \
	};                                                                                         \
	const static struct adc_rtl8752h_config adc_rtl8752h_config_##index = {                    \
		.reg = DT_INST_REG_ADDR(index),                                                    \
		.clkid = DT_INST_CLOCKS_CELL(index, id),                                           \
		.channels = DT_INST_PROP(index, channels),                                         \
		.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(index),                                     \
		.irq_num = DT_INST_IRQN(index),                                                    \
		.irq_config_func = adc_rtl8752h_irq_config_func,                                   \
	};                                                                                         \
	PM_DEVICE_DT_INST_DEFINE(index, adc_rtl8752h_pm_action);                                   \
	DEVICE_DT_INST_DEFINE(index, &adc_rtl8752h_init, PM_DEVICE_DT_INST_GET(index),             \
			      &adc_rtl8752h_data_##index, &adc_rtl8752h_config_##index,            \
			      POST_KERNEL, CONFIG_ADC_INIT_PRIORITY, &adc_rtl8752h_driver_api);

DT_INST_FOREACH_STATUS_OKAY(RTL8752H_ADC_INIT)
