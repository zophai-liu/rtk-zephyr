...
#define DT_DRV_COMPAT realtek_rtl8752h_pwm
...
static int pwm_rtl8752h_set_cycles(const struct device *dev, uint32_t channel,
				   uint32_t period_cycles, uint32_t pulse_cycles, pwm_flags_t flags)
{
	LOG_DBG("channel=%d, period_cycles=%x, pulse_cycles=%x, flags=%x\n", channel, period_cycles,
		pulse_cycles, flags);
	const struct pwm_rtl8752h_config *config = dev->config;
#ifdef CONFIG_PM_DEVICE
	struct pwm_rtl8752h_data *data = dev->data;
#endif
...
static int pwm_rtl8752h_get_cycles_per_sec(const struct device *dev, uint32_t channel,
					   uint64_t *cycles)
{
	struct pwm_rtl8752h_data *data = dev->data;
	const struct pwm_rtl8752h_config *config = dev->config;
...
#ifdef CONFIG_PM_DEVICE
static int pwm_rtl8752h_pm_action(const struct device *dev, enum pm_device_action action)
{
	const struct pwm_rtl8752h_config *config = dev->config;
	struct pwm_rtl8752h_data *data = dev->data;
	void *timer_base = (void *)config->reg;
...
static const struct pwm_driver_api pwm_rtl8752h_driver_api = {
	.set_cycles = pwm_rtl8752h_set_cycles,
	.get_cycles_per_sec = pwm_rtl8752h_get_cycles_per_sec,
};

static int pwm_rtl8752h_init(const struct device *dev)
{
	const struct pwm_rtl8752h_config *config = dev->config;
	struct pwm_rtl8752h_data *data = dev->data;
	void *timer_base = (void *)config->reg;
...
#define PWM_RTL8752H_INIT(index)                                                                   \
	static struct pwm_rtl8752h_data pwm_rtl8752h_data_##index;                                 \
                                                                                                   \
	PINCTRL_DT_INST_DEFINE(index);                                                             \
                                                                                                   \
	static const struct pwm_rtl8752h_config pwm_rtl8752h_config_##index = {                    \
		...
		.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(index),                                     \
                                                                                                   ...
                                                                                                   \
	PM_DEVICE_DT_INST_DEFINE(index, pwm_rtl8752h_pm_action);                                   \
	DEVICE_DT_INST_DEFINE(index, &pwm_rtl8752h_init, PM_DEVICE_DT_INST_GET(index),             \
			      &pwm_rtl8752h_data_##index, &pwm_rtl8752h_config_##index,            \
			      POST_KERNEL, CONFIG_PWM_INIT_PRIORITY, &pwm_rtl8752h_driver_api);

DT_INST_FOREACH_STATUS_OKAY(PWM_RTL8752H_INIT)
