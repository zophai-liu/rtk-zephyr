...
#define DT_DRV_COMPAT realtek_rtl8752h_timer
...
LOG_MODULE_REGISTER(counter_rtl8752h_timer, CONFIG_COUNTER_LOG_LEVEL);

struct counter_rtl8752h_ch_data {
	counter_alarm_callback_t callback;
	void *user_data;
};

struct counter_rtl8752h_data {
...
#endif
	struct counter_rtl8752h_ch_data alarm[];
};

struct counter_rtl8752h_config {
...
static int counter_rtl8752h_timer_start(const struct device *dev)
{
	const struct counter_rtl8752h_config *cfg = dev->config;

	LOG_DBG("counter start\n");
	if (cfg->enhanced) {
		LOG_DBG("enhed %s, ctrl=%x, current=%x, line%d\n", dev->name,
			((ENHTIM_TypeDef *)cfg->reg)->MAX_CNT,
		...
		LOG_DBG("not enhed %s, ctrl=%x, current=%x, line%d\n", dev->name,
			((TIM_TypeDef *)cfg->reg)->ControlReg,
...
static int counter_rtl8752h_timer_stop(const struct device *dev)
{
	const struct counter_rtl8752h_config *cfg = dev->config;
...
static int counter_rtl8752h_timer_get_value(const struct device *dev, uint32_t *ticks)
{
	const struct counter_rtl8752h_config *cfg = dev->config;
...
static uint32_t counter_rtl8752h_timer_get_top_value(const struct device *dev)
{
	const struct counter_rtl8752h_config *cfg = dev->config;

	if (cfg->enhanced) {
		LOG_DBG("enhed, topvalue=%x\n", ((ENHTIM_TypeDef *)cfg->reg)->MAX_CNT);
		return ((ENHTIM_TypeDef *)cfg->reg)->MAX_CNT;
	}

	LOG_DBG("not enhed, topvalue=%x\n", ((TIM_TypeDef *)cfg->reg)->LoadCount);
	return ((TIM_TypeDef *)cfg->reg)->LoadCount;
}

static int counter_rtl8752h_timer_set_alarm(const struct device *dev, uint8_t chan,
					    const struct counter_alarm_cfg *alarm_cfg)
{
	struct counter_rtl8752h_data *data = dev->data;
	struct counter_rtl8752h_ch_data *chdata = &data->alarm[chan];

	if (alarm_cfg->ticks > counter_rtl8752h_timer_get_top_value(dev)) {
		return -EINVAL;
	}
...
static int counter_rtl8752h_timer_cancel_alarm(const struct device *dev, uint8_t chan)
{
	struct counter_rtl8752h_data *data = dev->data;
...
static int counter_rtl8752h_timer_set_top_value(const struct device *dev,
...
static uint32_t counter_rtl8752h_timer_get_pending_int(const struct device *dev)
{
	const struct counter_rtl8752h_config *cfg = dev->config;
...
static uint32_t counter_rtl8752h_timer_get_freq(const struct device *dev)
{
	struct counter_rtl8752h_data *data = dev->data;
...
static uint32_t counter_rtl8752h_timer_get_guard_period(const struct device *dev, uint32_t flags)
{
	struct counter_rtl8752h_data *data = dev->data;
...
static int counter_rtl8752h_timer_set_guard_period(const struct device *dev, uint32_t guard,
						   uint32_t flags)
{
	struct counter_rtl8752h_data *data = dev->data;

	LOG_DBG("guard=%d\n", guard);
	__ASSERT_NO_MSG(guard < counter_rtl8752h_timer_get_top_value(dev));
...
static void alarm_irq_handle(const struct device *dev, uint32_t chan)
{
	LOG_DBG("chan=%d\n", chan);
	struct counter_rtl8752h_data *data = dev->data;
	struct counter_rtl8752h_ch_data *alarm = &data->alarm[chan];
	counter_alarm_callback_t cb;
	uint32_t ticks;

	counter_rtl8752h_timer_get_value(dev, &ticks);
	cb = alarm->callback;
	alarm->callback = NULL;
...
#ifdef CONFIG_PM_DEVICE
static int counter_rtl8752h_timer_pm_action(const struct device *dev, enum pm_device_action action)
{
	const struct counter_rtl8752h_config *cfg = dev->config;
	struct counter_rtl8752h_data *data = dev->data;
...
static int counter_rtl8752h_timer_init(const struct device *dev)
{
	const struct counter_rtl8752h_config *cfg = dev->config;
	struct counter_rtl8752h_data *data = dev->data;
	void *timer_base = (void *)cfg->reg;
	...
	if (cfg->enhanced) {
		...
		LOG_DBG("enhed %s, ctrl=%x, line%d\n", dev->name,
			((ENHTIM_TypeDef *)timer_base)->CR, __LINE__);
	} else {
		...
		LOG_DBG("not enhed %s, ctrl=%x, line%d\n", dev->name,
			((TIM_TypeDef *)timer_base)->ControlReg, __LINE__);
	}

	return 0;
}

static const struct counter_driver_api counter_rtl8752h_timer_driver_api = {
	.start = counter_rtl8752h_timer_start,
	.stop = counter_rtl8752h_timer_stop,
	.get_value = counter_rtl8752h_timer_get_value,
	.set_alarm = counter_rtl8752h_timer_set_alarm,
	.cancel_alarm = counter_rtl8752h_timer_cancel_alarm,
	.set_top_value = counter_rtl8752h_timer_set_top_value,
	.get_pending_int = counter_rtl8752h_timer_get_pending_int,
	.get_top_value = counter_rtl8752h_timer_get_top_value,
	.get_guard_period = counter_rtl8752h_timer_get_guard_period,
	.set_guard_period = counter_rtl8752h_timer_set_guard_period,
	.get_freq = counter_rtl8752h_timer_get_freq,
};
...
#define RTL8752H_TIMER_INIT(index)                                                                 \
	TIMER_IRQ_CONFIG(index);                                                                   \
	static struct timer_data_##index {                                                         \
		struct counter_rtl8752h_data data;                                                 \
		struct counter_rtl8752h_ch_data alarm[DT_INST_PROP(index, channels)];              \
	} counter_rtl8752h_data_##index = {0};                                                     \
	static const struct counter_rtl8752h_config counter_rtl8752h_config_##index = {            \
		.counter_info =                                                                    \
			{                                                                          \
		...
		.get_irq_pending = get_irq_pending_##index,                                        \
                                                                                                   ...
                                                                                                   \
	PM_DEVICE_DT_INST_DEFINE(index, counter_rtl8752h_timer_pm_action);                         \
	DEVICE_DT_INST_DEFINE(index, counter_rtl8752h_timer_init, PM_DEVICE_DT_INST_GET(index),    \
			      &counter_rtl8752h_data_##index, &counter_rtl8752h_config_##index,    \
			      PRE_KERNEL_1, CONFIG_COUNTER_INIT_PRIORITY,                          \
			      &counter_rtl8752h_timer_driver_api);

DT_INST_FOREACH_STATUS_OKAY(RTL8752H_TIMER_INIT);
