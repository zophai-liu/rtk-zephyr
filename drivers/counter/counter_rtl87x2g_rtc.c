...
static int counter_rtl87x2g_rtc_get_value(const struct device *dev, uint32_t *ticks)
{
...
static int counter_rtl87x2g_rtc_set_alarm(const struct device *dev, uint8_t chan,
...
static int counter_rtl87x2g_rtc_cancel_alarm(const struct device *dev, uint8_t chan)
{
	struct counter_rtl87x2g_rtc_data *data = dev->data;
...
static uint32_t counter_rtl87x2g_rtc_get_guard_period(const struct device *dev, uint32_t flags)
{
...
static int counter_rtl87x2g_rtc_set_guard_period(const struct device *dev, uint32_t guard,
						 uint32_t flags)
{
	LOG_DBG("guard=%d\n", guard);
	struct counter_rtl87x2g_rtc_data *data = dev->data;
...
#define RTL87X2G_RTC_INIT(index)                                                                   \
	...
	DEVICE_DT_INST_DEFINE(index, counter_rtl87x2g_rtc_init, NULL,                              \
			      &counter_rtl87x2g_rtc_data_##index,                                  \
			      &counter_rtl87x2g_rtc_config_##index, PRE_KERNEL_1,                  \
			      CONFIG_COUNTER_INIT_PRIORITY, &counter_rtl87x2g_rtc_driver_api);
