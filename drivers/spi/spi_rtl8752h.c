 ...
 */
#define DT_DRV_COMPAT realtek_rtl8752h_spi
...
LOG_MODULE_REGISTER(spi_rtl8752h, CONFIG_SPI_LOG_LEVEL);
#include "spi_context.h"

#ifdef CONFIG_SPI_RTL8752H_DMA

struct spi_rtl8752h_dma_data {
...
struct spi_rtl8752h_data {
	...
	struct spi_rtl8752h_dma_data dma_rx_data;
	struct spi_rtl8752h_dma_data dma_tx_data;
#endif
...
struct spi_rtl8752h_config {
	uint32_t reg;
	uint16_t clkid;
	const struct pinctrl_dev_config *pcfg;
#ifdef CONFIG_SPI_RTL8752H_INTERRUPT
	void (*irq_configure)();
#endif
};

static bool spi_rtl8752h_transfer_ongoing(struct spi_rtl8752h_data *data)
{
	return spi_context_tx_on(&data->ctx) || spi_context_rx_on(&data->ctx);
}

static int spi_rtl8752h_get_err(const struct spi_rtl8752h_config *cfg)
{
...
static int spi_rtl8752h_frame_exchange(const struct device *dev)
{
	struct spi_rtl8752h_data *data = dev->data;
	const struct spi_rtl8752h_config *dev_config = dev->config;
	...
	return spi_rtl8752h_get_err(dev_config);
}

#ifdef CONFIG_SPI_RTL8752H_INTERRUPT
static void spi_rtl8752h_complete(const struct device *dev, int status)
{
	struct spi_rtl8752h_data *dev_data = dev->data;
	const struct spi_rtl8752h_config *dev_config = dev->config;
	SPI_TypeDef *spi = (SPI_TypeDef *)dev_config->reg;
...
static void spi_rtl8752h_isr(struct device *dev)
{
	const struct spi_rtl8752h_config *cfg = dev->config;
	struct spi_rtl8752h_data *data = dev->data;
	int err = 0;

	err = spi_rtl8752h_get_err(cfg);
	if (err) {
		spi_rtl8752h_complete(dev, err);
		return;
	}

	if (spi_rtl8752h_transfer_ongoing(data)) {
		err = spi_rtl8752h_frame_exchange(dev);
	}

	if (err || !spi_rtl8752h_transfer_ongoing(data)) {
		spi_rtl8752h_complete(dev, err);
	}
}

#endif /* CONFIG_SPI_RTL8752H_INTERRUPT */

#ifdef CONFIG_SPI_RTL8752H_DMA
static int spi_rtl8752h_start_dma_transceive(const struct device *dev)
{
	const struct spi_rtl8752h_config *cfg = dev->config;
	struct spi_rtl8752h_data *data = dev->data;
...
static bool spi_rtl8752h_chunk_transfer_finished(const struct device *dev)
{
	struct spi_rtl8752h_data *data = dev->data;
	const size_t chunk_len = spi_context_max_continuous_chunk(&data->ctx);
...
void spi_rtl8752h_dma_rx_cb(const struct device *dev, void *user_data, uint32_t channel, int status)
{

	const struct device *dma_dev = (const struct device *)dev;
	const struct device *spi_dev = (const struct device *)user_data;
	struct spi_rtl8752h_data *data = spi_dev->data;
	const size_t chunk_len = spi_context_max_continuous_chunk(&data->ctx);
	uint32_t datalen = data->datasize;
	int dfs = ((datalen - 1) >> 3) + 1;
	int err = 0;
	...
	if (status < 0) {
		...
		spi_rtl8752h_complete(spi_dev, status);
		return;
	}
	...
	if (spi_rtl8752h_chunk_transfer_finished(spi_dev)) {
	...
	err = spi_rtl8752h_start_dma_transceive(spi_dev);
	if (err) {
		spi_rtl8752h_complete(spi_dev, err);
	}
}

void spi_rtl8752h_dma_tx_cb(const struct device *dev, void *user_data, uint32_t channel, int status)
{
}

#endif /* CONFIG_SPI_RTL8752H_DMA */

static int spi_rtl8752h_configure(const struct device *dev, const struct spi_config *spi_cfg)
{
	struct spi_rtl8752h_data *data = dev->data;
	const struct spi_rtl8752h_config *config = dev->config;
...
static int spi_rtl8752h_transceive_impl(const struct device *dev, const struct spi_config *spi_cfg,
					...
					spi_callback_t cb, void *userdata)
{
	...
	ret = spi_rtl8752h_configure(dev, spi_cfg);
	if (ret < 0) {
		goto error;
	}
	...
	if (data->dma_rx.dma_dev && data->dma_tx.dma_dev) {
		...
		ret = spi_rtl8752h_start_dma_transceive(dev);
		if (ret < 0) {
			goto dma_error;
		}
...
#else
	do {
		ret = spi_rtl8752h_frame_exchange(dev);
		if (ret < 0) {
			break;
		}
	} while (spi_rtl8752h_transfer_ongoing(data));
...
static int spi_rtl8752h_transceive(const struct device *dev, const struct spi_config *spi_cfg,
				   const struct spi_buf_set *tx_bufs,
				   const struct spi_buf_set *rx_bufs)
{
	return spi_rtl8752h_transceive_impl(dev, spi_cfg, tx_bufs, rx_bufs, false, NULL, NULL);
}

#ifdef CONFIG_SPI_ASYNC
static int spi_rtl8752h_transceive_async(const struct device *dev, const struct spi_config *spi_cfg,
					 const struct spi_buf_set *tx_bufs,
					 const struct spi_buf_set *rx_bufs, spi_callback_t cb,
					 void *userdata)
{
	return spi_rtl8752h_transceive_impl(dev, spi_cfg, tx_bufs, rx_bufs, true, cb, userdata);
}
#endif

static int spi_rtl8752h_release(const struct device *dev, const struct spi_config *config)
{
	struct spi_rtl8752h_data *data = dev->data;
...
#ifdef CONFIG_PM_DEVICE
static int spi_rtl8752h_pm_action(const struct device *dev, enum pm_device_action action)
{
	struct spi_rtl8752h_data *data = dev->data;
	const struct spi_rtl8752h_config *config = dev->config;
	SPI_TypeDef *spi = (SPI_TypeDef *)config->reg;
...
static const struct spi_driver_api spi_rtl8752h_driver_api = {
	.transceive = spi_rtl8752h_transceive,
#ifdef CONFIG_SPI_ASYNC
	.transceive_async = spi_rtl8752h_transceive_async,
#endif
	.release = spi_rtl8752h_release,
};

#ifdef CONFIG_SPI_RTL8752H_DMA
static int spi_rtl8752h_dma_init(const struct device *dev)
{
	struct spi_rtl8752h_data *data = dev->data;
	const struct spi_rtl8752h_config *config = dev->config;
	SPI_TypeDef *spi = (SPI_TypeDef *)config->reg;
...
static int spi_rtl8752h_init(const struct device *dev)
{
	struct spi_rtl8752h_data *data = dev->data;
	const struct spi_rtl8752h_config *cfg = dev->config;
	int ret;
	...
	.dma_cfg =                                                                                 \
		{                                                                                  \
			...
			.dest_burst_length = RTL8752H_DMA_CONFIG_DESTINATION_MSIZE(                \
				RTL8752H_DMA_CHANNEL_CONFIG(index, dir)),                          \
			.block_count = 1,                                                          \
			.dma_callback = spi_rtl8752h_dma_##dir##_cb,                               \
	},                                                                                         \
	.src_addr_increment =                                                                      \
...
#if defined(CONFIG_SPI_RTL8752H_DMA)
...
#define SPI_RTL8752H_IRQ_CONFIGURE(index)                                                          \
	static void spi_rtl8752h_irq_configure_##index(void)                                       \
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN(index), DT_INST_IRQ(index, priority), spi_rtl8752h_isr,   \
			    DEVICE_DT_INST_GET(index), 0);                                         \
		irq_enable(DT_INST_IRQN(index));                                                   \
	}

#define RTL8752H_SPI_INIT(index)                                                                   \
	PINCTRL_DT_INST_DEFINE(index);                                                             \
	IF_ENABLED(CONFIG_SPI_RTL8752H_INTERRUPT, (SPI_RTL8752H_IRQ_CONFIGURE(index)));            \
	static struct spi_rtl8752h_data spi_rtl8752h_data_##index = {                              \
		SPI_CONTEXT_INIT_LOCK(spi_rtl8752h_data_##index, ctx),                             \
		SPI_CONTEXT_INIT_SYNC(spi_rtl8752h_data_##index, ctx),                             \
	...
	static struct spi_rtl8752h_config spi_rtl8752h_config_##index = {                          \
		.reg = DT_INST_REG_ADDR(index),                                                    \
		.clkid = DT_INST_CLOCKS_CELL(index, id),                                           \
		.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(index),                                     \
		IF_ENABLED(CONFIG_SPI_RTL8752H_INTERRUPT,                                          \
			   (.irq_configure = spi_rtl8752h_irq_configure_##index))};                \
	PM_DEVICE_DT_INST_DEFINE(index, spi_rtl8752h_pm_action);                                   \
	DEVICE_DT_INST_DEFINE(index, &spi_rtl8752h_init, PM_DEVICE_DT_INST_GET(index),             \
			      &spi_rtl8752h_data_##index, &spi_rtl8752h_config_##index,            \
			      POST_KERNEL, CONFIG_SPI_INIT_PRIORITY, &spi_rtl8752h_driver_api);

DT_INST_FOREACH_STATUS_OKAY(RTL8752H_SPI_INIT)
