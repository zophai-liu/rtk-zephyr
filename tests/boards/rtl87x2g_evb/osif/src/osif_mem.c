...
ZTEST(osif_mem, test_os_mem_alloc)
{
	...
	uint32_t heap_itcm_free_size = os_mem_peek(RAM_TYPE_DATA_ON);

	/* Test the heap allocate Strategy: first heap_itcm, then heap_dtcm and finally heap_sram.
	 */
	if (heap_itcm_free_size >
	    0) { /*in case the lowstack and platform has run out of heap_itcm.*/
		iptr = os_mem_alloc(RAM_TYPE_DATA_ON, heap_itcm_free_size);
		zassert_not_null(
			(iptr),
			"os_mem_alloc failed with maxiumn size, type RAM_TYPE_DATA_ON, errno: %d",
			errno);
		zassert_true((uint32_t)iptr >=
				     (DT_REG_ADDR(DT_NODELABEL(tcm_heap)) &&
				      (uint32_t)iptr <= (DT_REG_ADDR(DT_NODELABEL(tcm_heap)) +
							 DT_REG_SIZE(DT_NODELABEL(tcm_heap)))),
